...
public class StickerFetchTask extends AsyncTask<Void, Void, Void> {

    private static final String API_URL = "http://api.giphy.com/";
    private static final String API_URI = "v1/stickers/";
    private static final String API_KEY = "dc6zaTOxFJmzC";
    private StickerFragment fragment;
    private String method = "trending";
    private String search = "";

    public StickerFetchTask(final StickerFragment fragment) {
        this.fragment = fragment;
        this.method = "trending";
    }

    public StickerFetchTask(final StickerFragment fragment, final String searchContent) {
        this.fragment = fragment;
        this.method = "search";
        try {
        ...
        if (AppConstants.NETWORK_ONLINE) {
            ...
            call.enqueue(new Callback() {
                ...
                public void onFailure(Request request, IOException e) {
                    Log.i("StickerFetchTask", "Request Failure");
                }
                            ...
                            for (int i = 0; i < count; i++) {
                                ...
                                if (!row.isNull("images")) {
                                    ...
                                    if (!imageData.isNull("fixed_width") && !imageData.isNull("fixed_width_still")) {
                                        ...
                                        newData.add(new MediaViewItem(animatedPath, stillPath, true));
                                    }
                                }
                            }
                            newData.add(new MediaViewItem(true));
                            fragment.updateAdapterData(newData);
                        } else {
                            Log.i("StickerFetchTask", "Response Unsuccessful " + jsonData);
                        }
                    } catch (IOException e) {
